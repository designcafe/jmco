...
if ($config->get('load_googlefonts', false)) {
	...
	$this->warp->stylesheets->add($httpmode.'://fonts.googleapis.com/css?family=Lobster&subset=latin');	
}

// javascripts
$this->warp->javascripts->addDeclaration('var Warp = Warp || {}; Warp.Settings = '.json_encode(array('color' => $config->get('color'), 'itemColor' => $config->get('itemcolor'))).';');
