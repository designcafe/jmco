 ...
 * @version $Id: akeebaui.js 434 2011-02-08 09:43:51Z nikosdion $
 **/
			...
			$.each(groupdef, function(config_key, defdata){
				...
				switch( defdata['type'] )
				{
					...
					case 'engine':
						...
						var button = $(document.createElement('input')).attr('type','button').val(akeeba_translations['UI-CONFIG']);
					...
					case 'browsedir':
						...
						var button = $(document.createElement('input')).attr('type','button').val(akeeba_translations['UI-BROWSE']);
					...
					case 'buttonedit':
						...
						var button = $(document.createElement('input')).attr('type','button').val(akeeba_translations[defdata['buttontitle']]);
						button.bind('click',function(event){
							event.preventDefault();
					...
					case 'button':
						...
						var editor = $(document.createElement('input')).attr('type','button').attr('id', current_id).val(labeltext);
						label.html('&nbsp;');
...
function start_timeout_bar(max_allowance, bias)
{
	(function($) {
		...
		.animate({
			...
			$('#response-timer div.color-overlay').animate({
