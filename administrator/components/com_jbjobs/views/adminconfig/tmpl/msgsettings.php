 ...
 * Author 		:	Faisel
 + Project		: 	Job site
 * File Name	:	views/adminconfig/tmpl/msgsettings.php
 * License		:	GNU General Public License version 3, or later
 ^ 
 * Description	: 	Private Message settings (jbjobs)
 ^ 
 * History		:	NONE
 * */
...
<legend><?php echo JText::_( 'Private Message Settings' ); ?></legend>
	<table class="admintable">
		...
		<tr>
			...
			<td>
				<input type="radio" name="individualChange" value="1"<?php echo ($allSame?"":" checked"); ?> onchange="document.getElementById('messageLimitDefaultBox').style.display='none';for(i=0;i<7;i++){document.getElementById('messageLimitDefaultBox'+i).style.display='';}" /><?php echo JText::_( 'YES' ); ?><br />
				<input type="radio" name="individualChange" value="0"<?php echo ($allSame?" checked":""); ?> onchange="document.getElementById('messageLimitDefaultBox').style.display='';for(i=0;i<7;i++){document.getElementById('messageLimitDefaultBox'+i).style.display='none';}" /><?php echo JText::_( 'NO' ); ?>
			</td>
		</tr>
		<tr id="messageLimitDefaultBox">
			<td class="key">
		...
		<?php
			...
			while(sizeof($this->types) > $curIndex){
				echo '<tr id="messageLimitDefaultBox'.$curIndex.'"><td class="key"><label for="messageLimit">';
	...
	<?php if($allSame){ ?>
	<script type='text/javascript'>document.getElementById('messageLimitDefaultBox').style.display='';for(i=0;i<7;i++){document.getElementById('messageLimitDefaultBox'+i).style.display='none';}</script>
	<?php }else{ ?>
	<script type='text/javascript'>document.getElementById('messageLimitDefaultBox').style.display='none';for(i=0;i<7;i++){document.getElementById('messageLimitDefaultBox'+i).style.display='';}</script>
	<?php } ?>
</fieldset>
<div class="clr"></div>

<input type="hidden" name="option" value="com_jbjobs" />
<input type="hidden" name="task" value="savemsgsettings" />
</form>
