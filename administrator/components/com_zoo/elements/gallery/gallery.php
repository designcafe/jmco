...
class ElementGallery extends ElementSimpleParams {
	...
	*/
	function hasValue() {
		...
		// init vars
		$directory = JPATH_ROOT.DS.trim($this->directory, '/').DS.trim($this->value, '/');				
	...
	*/
	function render($view = ZOO_VIEW_ITEM) {
		...
		$gallery_params->set('src', trim($this->directory, '/').'/'.trim($this->value, '/'));
	...
	*/
	function edit() {
		...
		// create html
		$html  = '<table>';
		$html .= JHTML::_('element.editrow', JText::_('Directory'), JHTML::_('control.selectdirectory', JPATH_ROOT.DS.trim($this->directory, '/'), false, $this->name.'_value', $this->value));
