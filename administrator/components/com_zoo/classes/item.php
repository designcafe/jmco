...
class Item extends YObject {
	...
	*/
	...
	/*
		...
		Parameters:
	        $catalog_id - Only categories of a catalog
			$published - Only published categories

		Returns:
			Array - category id's
	*/
	function getRelatedCategories($catalog_id = null, $published = false) {

		// get category objects
		$table  =& JTable::getInstance('category', 'Table');
		$result = $table->getByItemId($this->id, $published);
		if ($result === false) {
		...
		if ($catalog_id) {
			...
			foreach ($result as $key => $category) {
				if ($category->catalog_id == $catalog_id) {
					$filtered[] =& $result[$key];
				}
			}
