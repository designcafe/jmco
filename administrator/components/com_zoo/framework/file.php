...
class YFile {
	...
	*/	
	function output($file) {
		...
		$name = basename($file);
		$type = YFile::getContentType($name);
		$size = @filesize($file);
	...
	/*
		...
		Returns:
			Array - Directories
	*/
	function readDirectory($path, $prefix = '', $filter = false, $recursive = true) {
		...
		if (is_readable($path) && is_dir($path) && $handle = @opendir($path)) {
			while (false !== ($file = readdir($handle))) {
	            ...
	            if (is_dir($path.'/'.$file)) {
					...
					// read subdirectory
					$dirs[] = $prefix.$file;
	            	$dirs   = array_merge($dirs, YFile::readDirectory($path.'/'.$file, $prefix.$file.'/', $filter, $recursive));
	...
	/*
		...
		Returns:
			Array - Files
	*/
	function readDirectoryFiles($path, $prefix = '', $filter = false, $recursive = true) {
		...
		if (is_readable($path) && is_dir($path) && $handle = @opendir($path)) {
			while (false !== ($file = readdir($handle))) {
	            ...
	            if (is_dir($path.'/'.$file)) {
					...
					// read subdirectory
	            	$files = array_merge($files, YFile::readDirectoryFiles($path.'/'.$file, $prefix.$file.'/', $filter, $recursive));
