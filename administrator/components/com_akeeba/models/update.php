 ...
 * @version $Id: update.php 355 2010-12-24 11:32:41Z nikosdion $
 * @since 2.2
 */
...
class AkeebaModelUpdate extends JModel
{
	...
	private function lastUpdateCheck()
	{
		...
		$lastdate = AEPlatform::get_platform_configuration_option('lastupdatecheck', '2009-04-02');
	...
	private function getLatestVersion($force = false)
	{
		...
		if($inidata != false)
		{
			...
			if(AKEEBA_PRO)
			{
				$dlid = AEPlatform::get_platform_configuration_option('update_dlid', '');
				if( !empty($dlid) )
				{
					$suffix = '?dlid='.urlencode($dlid).'&format=raw';
				}
			}
	...
	{
		$inidata =  AEPlatform::get_platform_configuration_option('updateini', "");
