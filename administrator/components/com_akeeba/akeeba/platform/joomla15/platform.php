 ...
 * @version $Id: platform.php 427 2011-02-06 11:07:23Z nikosdion $
 */
...
class AEPlatform
{
	...
	public static function get_valid_remote_records($profile = null, $engine = null)
	{
		...
		$db =& AEFactory::getDatabase( self::get_platform_database_options() );
		$sql = 'SELECT '.$db->nameQuote('id').', '.$db->nameQuote('multipart').', '.
			$db->nameQuote('remote_filename').', '.$db->nameQuote('total_size').' '.
			' FROM '.$db->nameQuote('#__ak_stats').' WHERE '.
