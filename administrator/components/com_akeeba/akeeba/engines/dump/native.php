 ...
 * @version $Id: native.php 435 2011-02-08 10:07:29Z nikosdion $
 */
	...
	private function get_create( $table_abstract, $table_name, &$type, &$dependencies )
	{
		...
		if( in_array($type, array('table','merge','view')) )
		{
			...
			else
			{
				...
				// # Fix 3.2.1: TYPE=MyISAM, ENGINE=MyISAM would cause trouble on some idiotic MySQL servers which have been set up by bloody morons...
				$table_sql = str_replace('TYPE=MyISAM','',$table_sql);
				$table_sql = str_replace('ENGINE=MyISAM','',$table_sql);
			}
