 ...
 * @version $Id: default.php 407 2011-01-24 09:30:22Z nikosdion $
 * @since 1.3
 */
		...
		<table id="backup-setup-parameters" class="adminlist" width="100%">
			<tr class="row<?php $row = 1 - $row; echo $row ?>">
				...
				<td>
					...
					<input type="button" value="<?php echo JText::_('CPANEL_PROFILE_BUTTON'); ?>" onclick="flipProfile();" />
				</td>
			</tr>
		</table>		
	</form>
...
<textarea id="comment" rows="5" cols="73"><?php echo $this->comment ?></textarea>
		...
		<tr class="row<?php $row = 1 - $row; echo $row ?>">
			<td>&nbsp;</td>
			<td>
				<input id="backup-start" type="button" value="<?php echo JText::_('BACKUP_LABEL_START') ?>" />
			</td>
		</tr>
	</table>
	</form>
	...
	<fieldset>
		...
		<div id="finishedframe">
			...
			<div class="ak-action-button">
				...
				<input type="button"
					value="<?php echo JText::_('BUADMIN'); ?>"
					onclick="window.location='<?php echo JURI::base() ?>index.php?option=com_akeeba&view=buadmin';"
				/>
			</div>
			<div class="ak-action-button">
				<div class="ak-icon ak-icon-viewlog"></div>
				<input type="button"
					value="<?php echo JText::_('VIEWLOG'); ?>"
					onclick="window.location='<?php echo JURI::base() ?>index.php?option=com_akeeba&view=log';"
				/>
			</div>
		</div>
	</fieldset>
